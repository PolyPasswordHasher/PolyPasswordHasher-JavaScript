...
var PPH = function (threshold, passwordfile, isolatedcheckbits){
	...
	if (typeof passwordfile === 'undefined'){
		...
		this.shamirsecretshares = sss.share(this.shieldedkey,this.threshold,this.threshold);
	...
	if (typeof this.accountdict !== "object" || typeof this.secretintegritycheck !== "string")
		throw new Error("Password file did not load correctly; could not deserialize data.");
	...
	for (var user=0; user<this.accountdict.length; user++)
		for (var share=0; share<this.accountdict[user].length; share++)
			this.nextavailableshare = Math.max(this.accountdict[user][share]["sharenumber"],
												nextavailableshare);
...
PPH.prototype.create_account = function (username, password, shares) {
	...
	if ((shares + this.nextavailableshare) > 255)
		throw new Error("This ("+shares+") number of shares would exceed the max (255)");
	...
	if (!this.knownsecret){
		...
		thisentry.salt = sss.random(this.saltsize*8); //multiply by 8 bc sss.random() takes bits
		var tempsaltedpasshash = _SHA256(thisentry["salt"]+password);
		thisentry.saltedpasshash = _AESencrypt(this.shieldedkey, tempsaltedpasshash);
		thisentry.saltedpasshash += this.create_isolated_validation_bits(tempsaltedpasshash);

		this.accountdict[username].push(thisentry);
		return;
	}

	for (var sharenumber=this.nextavailableshare; sharenumber<this.nextavailableshare+shares; sharenumber++){
		thisentry = {};
		thisentry.sharenumber = sharenumber;

		var shamirsecretshare = sss.newShare(sharenumber, this.shamirsecretshares).substring(3);
		thisentry.salt = sss.random(this.saltsize*8);
		var tempsaltedpasshash = _SHA256(thisentry["salt"]+password);
...
PPH.prototype.is_valid_login = function (username, password) {
	...
	for (var entry=0; entry<this.accountdict[username].length; entry++){
		tempsaltedpasshash = _SHA256(this.accountdict[username][entry]["salt"]+password);
		...
		//XOR to remove saltedpasshash from the hashXORshare (i.e. to get share)
		var tempsharedata = _xor(tempsaltedpasshash, saltedpasshash.substring(0,saltedpasshash.length - this.isolatedcheckbits));

		var isolatedcheck;
		if (this.isolatedcheckbits > 0)
			isolatedcheck = this.isolated_validation(tempsaltedpasshash,saltedpasshash);
		else
			isolatedcheck = false;
		...
		if (tempsharenumber_dec === 0){
			...
			var tempSPH = saltedpasshash.substring(0, saltedpasshash.length - this.isolatedcheckbits);
			if (tempAES === tempSPH)
				return true;
			
			if (isolatedcheck)
				console.log("Isolated check matches but full hash doesn't; this might be a break-in!");
			return false;
		}
...
PPH.prototype.write_password_data = function (passwordfile) {
	...
	if (this.threshold >= this.nextavailableshare)
		throw new Error("Would write undecodable password file. Must have more shares before writing.");
	...
	var shamirsecretsharesbackup = JSON.parse(JSON.stringify(this.shamirsecretshares));
...
* [["ecr","issleepdeprived"],["bernie","willneverwin"],["konami","upupdowndown"]]
*/
PPH.prototype.unlock_password_data = function (logindata) {
	...
	for (var login=0; login<logindata.length; login++){
		...
		for (var entry=0; entry<this.accountdict[username].length; entry++){
			...
			var tempshare = "8"+tempsharenumber_hex+_xor(tempsaltedpasshash,
								...
								tempentry["saltedpasshash"].length - this.isolatedcheckbits));
	...
	if (!(this.verify_secret(combined)))
		throw new Error("This is not a valid secret recombination. Inadequate/incorrect shares provided.");
	...
	for (entry=0; entry<this.bootstrapaccounts.length; entry++){
		...
		tempentry["saltedpasshash"] = _AESencrypt(this.shieldedkey,tempentry["saltedpasshash"]);
...
PPH.prototype.isolated_validation = function (temphash, storedhash) {
	...
	var localICB = storedhash.substring(storedhash.length - this.isolatedcheckbits);
	return saltedpasshashICB === localICB;
};

/**
* Creates and returns a random string consisting of 28 bytes of random data and 3 bytes
* of hash to verify the secret upon recombination
...
PPH.prototype.create_secret = function () {
	...
	//makes random 31-byte string 
	var secret = sss.random(this.secretlength*8); //multiply by 8 bc random() takes bits
...
* @param secret
* @return a boolean that indicates whether the secret falls under the fingerprint
*/
...
/**
* Creates and returns the isolated-check bits suffix to add to an existing saltedpasshash
* @param saltedpasshash
* @return the suffix
*/
...
var _xor = function (x, y){
	...
	if (typeof x !== typeof y || typeof x !== "string" || typeof y !== "string")
		throw new Error(x+" ("+typeof x+") and/or "+y+" ("+typeof y+") are/is not of type string");
	...
	for (var i=0; i<xb.length; i++){
		...
		if (temp.length === 1)	result += ("0"+temp);
		else 					result += temp;
	}
	return result;
};
